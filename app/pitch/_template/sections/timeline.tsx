...
import type { Milestone, Pitch } from "@/pitches/types";
 ...
 */
export function Timeline({ index, data }: TimelineProps) {
  ...
  const ref = useGsapSection<HTMLElement>(
    ({ root, q, isDesktop }) => {
      ...
      const counter = (item: HTMLElement) => {
        ...
        const m = milestones[Number(item.dataset.index)];
    ...
    >
      ...
      <div className="track-viewport mt-14 w-full md:mt-20 md:pl-[max(1.5rem,calc((100vw-64rem)/2+1.5rem))]">
        <ol className="track relative flex flex-col gap-12 px-6 md:w-max md:flex-row md:gap-0 md:px-0 md:pr-[40vw]">
          <div
            className="pointer-events-none absolute top-1.5 right-0 left-0 hidden h-px bg-(--pitch-line) md:block"
            aria-hidden
          >
            <div className="rail-fill h-full w-full bg-(--pitch-accent)" />
          </div>
          <div
            className="pointer-events-none absolute top-2 bottom-2 left-[calc(1.5rem+5px)] w-px bg-(--pitch-line) md:hidden"
            aria-hidden
          >
            <div className="rail-v h-full w-full bg-(--pitch-accent)" />
          </div>
          {milestones.map((m, i) => (
            <li
              key={m.date}
            ...
            >
              <span
                className="dot relative mt-1 block size-3 shrink-0 rounded-full border-2 border-background bg-(--pitch-line) md:absolute md:top-0 md:left-0 md:mt-0 md:border-0"
                aria-hidden
              />
              <div className="milestone-body md:mt-8">
                ...
                <p className="mt-3 max-w-xs text-base text-(--pitch-ink-dim)">{m.label}</p>
              </div>
            </li>
          ))}
        </ol>
